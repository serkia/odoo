...
class WebsiteBlog(http.Controller):
    ...
    def nav_list(self):
        groups = request.env['blog.post'].read_group([], ['name', 'create_date'],
    ...
    def blog_post(self, blog, blog_post, tag_id=None, page=1, enable_editor=None, **post):
        ...
        tag = None
        if tag_id:
            tag = blog_tag_obj.search([('id', '=', int(tag_id))])
        ...
        if not blog_ranking:
            blog_ranking =  blog_post_obj.search_read([('id', '!=', blog.id)], ['ranking'])
        # use search_read and sort because orderby on compute field store=False not working
        sorted_list = sorted(blog_ranking, key=lambda k: k['ranking'], reverse=True)
        next_post = blog_post_obj.search([('id', '=', sorted_list[0]['id'])])
        values = {
            'tags': tags,
            'tag': tag,
            'blog': blog,
    ...
    def _blog_post_message(self, user, blog_post_id=0, **post):
        blog_post = request.env['blog.post'].sudo()

        if request.env.uid != request.website.user_id.id:
            partner_ids = [user.partner_id.id]
        else:
            partner_ids = blog_post._find_partner_from_emails(0, [post.get('email')])
            if not partner_ids or not partner_ids[0]:
                partner_ids = [request.env['res.partner'].sudo().create({'name': post.get('name'), 'email': post.get('email')}).id]

        blog_post = blog_post.search([('id', '=', int(blog_post_id))])
        message_id = blog_post.with_context(mail_create_nosubcribe=True).message_post(
    ...
    def post_discussion(self, blog_post_id, **post):
        ...
        message = request.env['mail.message'].sudo().search([('id', '=', id)])
        return self._get_discussion_detail(message, publish)

    @http.route('/blogpost/new', type='http', auth="public", website=True)
    def blog_post_create(self, blog_id, **post):
        new_blog_post = request.env['blog.post'].with_context(mail_create_nosubcribe=True).create({
            'blog_id': blog_id,
    ...
    def blog_post_copy(self, blog_post_id, **post):
        ...
        """
        blog_post_obj = request.env['blog.post'].search([('id', '=', int(blog_post_id))])
        new_blog_post = blog_post_obj.with_context(mail_create_nosubcribe=True).copy()
        return werkzeug.utils.redirect("/blog/%s/post/%s?enable_editor=1" % (slug(new_blog_post.blog_id), slug(new_blog_post)))
    ...
    def change_bg(self, post_id=0, image=None, **post):
        if not post_id:
            return False
        blog_post = request.env['blog.post'].search([('id', '=', int(post_id))])

        ...
        */
        on_expandable: function (event) {
            ...
            self.parent_thread.message_fetch(this.domain, this.context, false, function (arg, data) {
                ...
                self.parent_thread.append_messages(data, self.$el);
    ...
    mail.Thread = session.web.Widget.extend({
        ...
        */
        ...
        render_thread: function(messages,dom_insert_after, prepend){
            ...
            if (dom_insert_after && dom_insert_after.parent()[0] == self.$el[0]) {
                _(messages).each(function(message){ message.insertAfter(dom_insert_after)});
            } else if (prepend) {
                _(messages).each(function(message){ message.prependTo(self.$el)});
            } else {
                _(messages).each(function(message){ message.appendTo(self.$el); });
            }
        },
        ...
        */
        ...
        create_thread: function(records, dom_insert_after) {
            ...
            var messages = _.without(_.union(group_records.false, group_records.null), undefined)

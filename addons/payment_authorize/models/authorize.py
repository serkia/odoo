...
class PaymentAcquirerAuthorize(models.Model):
    ...
    def authorize_form_generate_values(self, partner_values, tx_values):
        ...
        temp_authorize_tx_values = {
            ...
            'state': partner_values['state'].name if partner_values['state'] else '',
        }
        if authorize_tx_values.get('return_url'):
            temp_authorize_tx_values['returndata'] = '%s' % authorize_tx_values.pop('return_url')
        else:
            temp_authorize_tx_values['returndata'] = ''

...
class crm_case_section(osv.osv):
    ...
    def _get_sale_orders_data(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for id in ids:
            ...
            res[id] = {
                'monthly_quoted': self.__get_bar_values(cr, uid, obj, created_domain, ['amount_total', 'date_order'], 'amount_total', 'date_order', context=context),
                'monthly_confirmed': self.__get_bar_values(cr, uid, obj, validated_domain, ['amount_total', 'date_order'], 'amount_total', 'date_order', context=context),
            }
        return res
    ...
    def _get_invoices_data(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for id in ids:
            ...
            res[id] = self.__get_bar_values(
                cr, uid, obj, created_domain, ['price_total', 'date'],
                'price_total', 'date', context=context)
        return res

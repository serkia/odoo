...
instance.web.form.KanbanSelection = instance.web.form.FieldChar.extend({
    ...
    prepare_dropdown_selection: function() {
        ...
        var fields_to_read = _.map(this.options && this.options.states_legend || {}, function (value, key, list) { return value; });
        ...
        if (legend_field && fields_to_read && stage_id) {
            var fetch_stage = new openerp.web.DataSet(this, self.view.fields[legend_field].field.relation).read_ids([stage_id], fields_to_read);
        }
        else { var fetch_stage = $.Deferred().resolve(false); }
        return $.when(fetch_stage).then(function (results) {
            _.map(selection, function(res) {
                ...
                if (results && results[0][self.options.states_legend[res[0]]]) {
                    value['state_name'] = results[0][self.options.states_legend[res[0]]];
                }
    ...
    render_value: function() {
        ...
        var dd_fetched = self.prepare_dropdown_selection();;

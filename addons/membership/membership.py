    ...
    def _get_membership(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for partner_id in  ids:
            ...
            if 'count_membership' in field_name:
                count = membership_line_obj.search_count(cr, uid, [('partner', '=', partner_id)], context=context)
                res[partner_id]['count_membership'] = count
            if 'current_membership_id' in field_name:
                current_partner_id = membership_line_obj.search(cr, uid, [('partner', '=', partner_id),('date_from', '<=', fields.date.today()),('date_to', '>=', fields.date.today())], limit=1, context=context)
                res[partner_id]['current_membership_id'] = membership_line_obj.browse(cr, uid, current_partner_id, context=context).membership_id
        return res

    def return_action_to_open(self, cr, uid, ids, context=None):
        res = self.pool.get('ir.actions.act_window').for_xml_id(cr, uid ,'membership','action_membership_product_count_view', context=context)
...
class Product(osv.osv):
    ...
    _columns = {
        ...
        'membership_duration': fields.integer('Membership Duration', help='Duration until which membership remains active.'),
    }
...
class account_invoice_line(osv.osv):
    ...
    def _create_membership_line(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            if line.invoice_id.type == 'out_invoice':
                ml_ids = member_line_obj.search(cr, uid, self._prepare_domain_invoice_membeship(line), context=context)
                if line.product_id and line.product_id.membership and not ml_ids:
                    # Product line has changed to a membership product
                    date_from = fields.datetime.now()
                    date_to = (datetime.strptime(date_from,tools.DEFAULT_SERVER_DATETIME_FORMAT).date()) + (timedelta(int(line.product_id.membership_duration)*365/12))
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        self._create_membership_line(cr, uid, [result], context=context)
        return result

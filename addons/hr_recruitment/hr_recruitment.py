...
class hr_recruitment_stage(osv.osv):
    ...
    def _get_default_job_ids(self, cr, uid, context={}):
        if context.get('default_job_id'):
            return [context.get('default_job_id')]
        return None

    _defaults = {
        'sequence': 1,
        'job_ids': _get_default_job_ids,
    }
...
class hr_applicant(osv.Model):
    ...
    def _get_default_stage_id(self, cr, uid, context=None):
        ...
        return self.stage_find(cr, uid, [], context.get('default_job_id'), [('fold', '=', False)], context=context)
    ...
    def _read_group_stage_ids(self, cr, uid, ids, domain, read_group_order=None, access_rights_uid=None, context=None):
        ...
        job_id = context.get('default_job_id')
        search_domain = []
        if job_id:
    ...
    _columns = {
        ...
        'stage_id': fields.many2one ('hr.recruitment.stage', 'Stage', track_visibility='onchange',
                        domain="[('job_ids', '=', job_id)]", copy=False),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('job_id') or context.get('default_job_id'):
            ...
            vals.update(self.onchange_job(cr, uid, [], job_id, stage_id, context=context)['value'])
        if vals.get('user_id'):

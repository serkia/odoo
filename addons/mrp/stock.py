...
class StockMove(osv.osv):
    ...
    def _calculate_cost_price(self, cursor, user, move, product, bom_id, bom_properties=None, context=None):
        ...
        for sub_product_dict in sub_products:
            ...
            sub_bom_id = bom_obj._bom_find(cursor, user, sub_product.id, properties=bom_properties)
            std_price = 0.0
            if sub_bom_id: # recursive call in case of BOM of sub product
                std_price += self._calculate_cost_price(cursor, user, move, sub_product, sub_bom_id, bom_properties=bom_properties, context=context)
            else:
    ...
    def get_price_unit(self, cr, uid, move, context=None):
        ...
        bom_id = self.pool.get('mrp.bom')._bom_find(cr, uid, product.id)
        if not bom_id: # no BoM: use standard_price
            return move.price_unit or move.product_id.standard_price
        
        return self._calculate_cost_price(cr, uid, move, product, bom_id, context)

...
class task(osv.osv):
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False, lazy=True, options=None):
        ...
        if project_id:
            ...
            options.update({
                'display_empty_columns':'stage_id, user_id',
                'domain': search_domain
            })
        return super(task, self).read_group(cr, uid, domain, fields, groupby, offset=offset, limit=limit, context=context, orderby=orderby, options=options)

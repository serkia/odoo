...
def _initialize_db(id, db_name, demo, lang, user_password):
    try:
        ...
        with closing(db.cursor()) as cr:
            ...
            odoo.tools.config['lang'] = lang
            cr.commit()

        registry = odoo.modules.registry.RegistryManager.new(
...
def _set_pg_password_in_environment(func):
    ...
    def wrapper(*args, **kwargs):
        if os.environ.get('PGPASSWORD') or not odoo.tools.config['db_password']:
            return func(*args, **kwargs)
        else:
            os.environ['PGPASSWORD'] = odoo.tools.config['db_password']
            try:
                return func(*args, **kwargs)
            finally:
...
def dump_db(db, stream):
    ...
    with odoo.tools.osutil.tempdir() as dump_dir:
        ...
        if odoo.tools.config['db_user']:
            cmd.append('--username=' + odoo.tools.config['db_user'])
        if odoo.tools.config['db_host']:
            cmd.append('--host=' + odoo.tools.config['db_host'])
        if odoo.tools.config['db_port']:
            cmd.append('--port=' + str(odoo.tools.config['db_port']))
        cmd.append(db)

        if odoo.tools.exec_pg_command(*cmd):
...
def restore_db(db, dump_file, copy=False):
    ...
    with odoo.tools.osutil.tempdir() as dump_dir:
        ...
        args = []
        if odoo.tools.config['db_user']:
            args.append('--username=' + odoo.tools.config['db_user'])
        if odoo.tools.config['db_host']:
            args.append('--host=' + odoo.tools.config['db_host'])
        if odoo.tools.config['db_port']:
            args.append('--port=' + str(odoo.tools.config['db_port']))
        ...
        registry = odoo.modules.registry.RegistryManager.new(db)
        with registry.cursor() as cr:
            if copy:
...
def exp_rename(old_name, new_name):
    odoo.modules.registry.RegistryManager.delete(old_name)
...
def exp_list(document=False):
    if not odoo.tools.config['list_db'] and not document:
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        odoo.modules.registry.RegistryManager.new(db, force_demo=False, update_module=True)
    return True

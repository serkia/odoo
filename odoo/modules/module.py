...
class AddonsHook(object):
    ...
    def load_module(self, name):
        ...
        sys.modules[new_name] = new_mod
        sys.modules[old_name] = ModuleProxy(old_name, new_mod, self)

        # execute source in context of module *after* putting everything in
        # sys.modules
        execfile(opj(path, '__init__.py'), new_mod.__dict__)
...
class OdooHook(object):
    ...
    def load_module(self, name):
        ...
        # just create a proxy module to canonical version
        sys.modules[name] = ModuleProxy(name, mod, self)

        return sys.modules[name]

class ModuleProxy(types.ModuleType):
    def __init__(self, name, module, loader):
        # sets __name__
        super(ModuleProxy, self).__init__(name)
        self._set('_module', module)
        self._set('__loader__', loader)
        # get from _module: __file__, __path__

    def _set(self, name, value):
        super(ModuleProxy, self).__setattr__(name, value)
